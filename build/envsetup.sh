...
- hzgerrit:   A Git wrapper that fetches/pushes patch from/to hz Gerrit Review.
- hzrebase:   Rebase a Gerrit change and push it again.
- hzremote:   Add git remote for hz Gerrit Review.
- aospremote:      Add git remote for matching AOSP repository.
- cafremote:       Add git remote for matching CodeAurora repository.
- githubremote:    Add git remote for hz Github.
...
function hzremote()
{
    ...
    then
        ...
        local PFX="HZOS-Project/"
    else
        local PROJECT=$REMOTE
    fi

    local HZ_USER=$(git config --get review.review.hzos.com.username)
    if [ -z "$HZ_USER" ]
    then
        git remote add hz ssh://review.hzos.com:29418/$PFX$PROJECT
    else
        git remote add hz ssh://$HZ_USER@review.hzos.com:29418/$PFX$PROJECT
    fi
...
function githubremote()
{
    ...
    git remote add github https://github.com/HZOS-Project/$PROJECT
...
function hzrebase() {
    ...
    git fetch "http://review.hzos.com/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
