...
hz_local_manifest = ".repo/local_manifests/hz.xml"
hz_default_revision =  os.getenv('ROOMSERVICE_DEFAULT_BRANCH', '14')
hz_dependencies = "hz.dependencies"
org_manifest = "hz-devices"  # leave empty if org is provided in manifest
...
def get_from_manifest(device_name):
    if os.path.exists(hz_local_manifest):
        man = load_manifest(hz_local_manifest)
...
def add_to_manifest(repos, fallback_branch=None):
    lm = load_manifest(hz_local_manifest)
    ...
    for repo in repos:
        ...
        if 'branch' in repo:
            ...
            repo_branch=hz_default_revision
        if 'remote' in repo:
...
def fetch_dependencies(repo_path, fallback_branch=None):
    ...
    dep_p = '/'.join((repo_path, hz_dependencies))
    ...
    for dependency in dependencies:
        if not is_in_manifest(dependency['target_path']):
            if not dependency.get('branch'):
                dependency['branch'] = hz_default_revision
...
def detect_revision(repo):
    ...
    print("Calculated revision: %s" % hz_default_revision)

    if has_branch(result, hz_default_revision):
        return hz_default_revision

    print("Branch %s not found" % hz_default_revision)
    sys.exit()
